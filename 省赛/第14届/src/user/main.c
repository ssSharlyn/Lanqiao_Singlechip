...
/* AD/DA */
idata unsigned int volt_1_x100; // 光敏电阻电压值x100
...
unsigned char temperature_cllct; // 温度采集值
unsigned char humi_cllct; // 湿度采集值
bit valid_flag = 0; // 数据有效标志
...
void seg_task(void) {	
	switch(seg_mode) {
		...
		case 3:
		  ...
		  // 温度数据
		  seg_buf[3] = temperature_cllct / 10 % 10;
		  seg_buf[4] = temperature_cllct % 10;
			seg_buf[5] = 11;
		  // 湿度数据有效
		  if(valid_flag) {
		    seg_buf[6] = humi_cllct / 10 % 10;
			  seg_buf[7] = humi_cllct % 10;
			}
			// 湿度数据无效
			else {
...
void led_task(void) {
	...
	ucled[4] = (humi_cllct == 255); // 采集到无效湿度数据时，LED5点亮
...
void ad_da_task(void) {
	...
	volt_1_x100 = ad_read(0x41) * 100 / 51; // 读取光敏电阻电压值
	...
	if(lit_flag == 0 && lit_flag_old == 1) {
	  ...
	  cllct_lock_flag = 1; // 采集锁定
		timer_3s = 0;   // 重置3s计时
		
		seg_mode_rcd = seg_mode; // 记录切换前的界面
		seg_mode = 3;   // 切换到温湿度界面
		
		temperature_cllct = temperature; // 温度始终需要采集
		humi_cllct = humi;
		
		// 触发采集，对有效数据进行处理
	  if(valid_flag == 1 && cllct_flag == 1) {   	
      cllct_num++; // 采集次数+1
      memcpy(ucRTC_rcd, ucRTC, 2); // 记录采集时的时间
	  	
	    // --- 温度数据处理 ---
	    temperature_max = temperature_cllct > temperature_max ? temperature_cllct : temperature_max; // 计算最大温度 
	    temperature_sum += temperature_cllct; // 计算温度总和
      temperature_ave_x10 = temperature_sum * 10 / cllct_num; // 计算平均温度
	    
	    // --- 湿度数据处理 ---  
      humi_max = humi_cllct > humi_max ? humi_cllct : humi_max; // 计算最大湿度 
	    humi_sum += humi_cllct; // 计算湿度总和
      humi_ave_x10 = humi_sum * 10 / cllct_num; // 计算平均湿度
	    
	    // 当采集到的数据均升高时，拉高标志位
	    if(cllct_num >= 2 && temperature_cllct > temperature_rcd && humi_cllct > humi_rcd) {
	    	data_flag = 1;
	    }
	    else {
	    	data_flag = 0;
	    ...
	    // 更新上一次采集到的数据记录
      temperature_rcd = temperature_cllct;
	    humi_rcd = humi_cllct;
	  }
	}	
...
void main(void) {
	...
	read_temperature(); // 上电时读一下温度，防止后面读到85度异常值
	timer0_init();
	timer1_init();
	scheduler_init();
	while(1) {
